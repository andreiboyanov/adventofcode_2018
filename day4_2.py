        ...
        for record in day:
            for minute in range(record[1], record[2]):
                if minute in minutes:
                    ...
                    minutes[minute] = 1
    most_asleep_minute = sorted(minutes.items(), key=lambda minute_record: minute_record[1], reverse=True)
    if most_asleep_minute:
        most_asleep_minute = most_asleep_minute[0]
    else:
        most_asleep_minute = (-1, -1)
    guards2[guard_id] = most_asleep_minute

print(sorted(guards2.items(), lambda guard: guard[1][1], reverse=True))
